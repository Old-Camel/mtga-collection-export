...
BULK_CACHE_FILE = "scryfall_arena_cache.json"
BULK_CACHE_MAX_AGE = 86400  # 24 小时


def get_bulk_arena_map():
    """返回 {arena_id: {'name': ..., 'type_line': ...}} 字典，有缓存则直接加载"""
    if os.path.exists(BULK_CACHE_FILE):
        age = time.time() - os.path.getmtime(BULK_CACHE_FILE)
        if age < BULK_CACHE_MAX_AGE:
            print("Loading cached Scryfall data...")
            with open(BULK_CACHE_FILE, 'r', encoding='utf-8') as f:
                raw = json.load(f)
            return {int(k): v for k, v in raw.items()}  # JSON key 是 str，转回 int

    # 获取 bulk data 下载地址
    print("Fetching Scryfall bulk data index...")
    req = urllib.request.Request(
        "https://api.scryfall.com/bulk-data",
        headers={'User-Agent': 'MTGA-Export-Py/1.0', 'Accept': 'application/json'}
    )
    with urllib.request.urlopen(req, timeout=10) as resp:
        index = json.loads(resp.read().decode())

    uri = next(x['download_uri'] for x in index['data'] if x['type'] == 'default_cards')

    # 流式下载，显示进度
    print("Downloading Scryfall bulk data (first run, ~100MB)...")
    req = urllib.request.Request(uri, headers={'User-Agent': 'MTGA-Export-Py/1.0', 'Accept': 'application/json'})
    tmp = BULK_CACHE_FILE + ".tmp"
    with urllib.request.urlopen(req, timeout=120) as resp:
        ...
        with open(tmp, 'wb') as f:
            while True:
                ...
                total_mb = f" / {total_size/1024/1024:.0f}MB" if total_size else ""
                print(f"\r  {mb:.1f}MB{total_mb}", end='', flush=True)
    print()

    # 解析，只保留有 arena_id 的卡牌
    print("Indexing arena cards...")
    ...
    print(f"Indexed {len(arena_map)} arena cards.")
    return arena_map
...
def main():
    ...
    for i, (grp_id, owned) in enumerate(sorted_cards, 1):
        card = arena_map.get(grp_id)
        if not card:
            unknown_count += 1
            continue
        name = card['name']
